...
import com.example.network.utils.Constants.BIRTHDAY_FIELD
...
class GetPerson @Inject constructor(
    ...
    suspend fun getPersonsByBirthday(birthday: String, page: Int = 1): List<Person> {
        val queryParameters = mapOf(
            PAGE_FIELD to page.toString(),
            SORT_FIELD to COUNT_AWARDS_FIELD,
            SORT_TYPE to SORT_DESC,
            BIRTHDAY_FIELD to birthday
        )
